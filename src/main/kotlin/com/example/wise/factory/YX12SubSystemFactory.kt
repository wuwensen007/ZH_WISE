...
class YX12SubSystemFactory: SubSystemFactory() {
    ...
    companion object{
        ...
        val list = mutableListOf<SubSystem>()

        init {
            val mesProductList: List<MESProduct> = CsvToBeanBuilder<MESProduct>(FileReader("""${Constant.LOCAL_WISE_BASE_PATH}${File.separator}${Constant.YX8CSVNAME}""")).withType(MESProduct::class.java).build().parse()
            ...
            mesProductList.forEach {
                ...
                list.add(when(it.systemName.toUpperCase()){
                    ...
                    "SPC" -> {
                        ...
                        DesktopSubSystem(systemName = it.systemName, systemVersion = SimpleStringProperty(it.version), updateTime = it.updateTime, runCommand = """cmd /c cd ${YX12SubSystemVer.getLocalSubSystemBasePath()}${File.separator}${Constant.SPC}&&${it.fileName}""", factoryVer = YX8SubSystemVer, systemProp = systemProp)
                    }
